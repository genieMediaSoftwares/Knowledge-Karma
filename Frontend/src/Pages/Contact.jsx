...
const Field = ({ label, icon, ...props }) => (
  <div className="flex flex-col gap-1.5 w-full">
    <label className="text-[11px] font-heading font-semibold uppercase tracking-wider text-white/40">
      {label}
    </label>
    <div className="relative">
      ...
      <input
        {...props}
        className="w-full pl-10 pr-4 py-3 rounded-xl text-sm text-white placeholder-white/30 bg-white/5 border border-white/10 outline-none transition-all duration-200 focus:border-accent focus:ring-2 focus:ring-accent/30 font-sans"
      />
    </div>
  </div>
);
...
const TextareaField = ({ label, icon, ...props }) => (
  <div className="flex flex-col gap-1.5 w-full">
    ...
    <div className="relative">
      ...
      <textarea
        {...props}
        className="w-full pl-10 pr-4 py-3 rounded-xl text-sm text-white placeholder-white/30 bg-white/5 border border-white/10 outline-none transition-all duration-200 focus:border-accent focus:ring-2 focus:ring-accent/30 resize-none font-sans"
      />
    </div>
  </div>
);
    ...
    <div className="w-full font-sans">
      ...
      >
        ...
        <div className="relative max-w-6xl mx-auto px-5 sm:px-8 pt-14 pb-0 flex flex-col lg:flex-row gap-10 items-end">
          ...
          <div className="flex-1 pb-14">
            ...
            />
            <p className="text-xs font-heading font-bold tracking-[2px] uppercase text-white/60 mb-4">
              Get in Touch
            </p>
            <h1 className="text-4xl sm:text-5xl font-heading font-extrabold leading-tight text-white mb-4">
              Let's talk about<br />
              <em className="not-italic text-white/80">what's possible</em>
            </h1>
            <p className="text-base text-white/55 leading-relaxed max-w-sm mb-8 font-sans">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <a
                href={`mailto:${EMAIL}`}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-white text-sm font-semibold hover:-translate-y-0.5 transition-all bg-white/10 hover:bg-white/20 border border-white/20"
              >
          ...
          >
            <h2 className="text-white text-xl font-heading font-bold mb-1">Send a message</h2>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {INTENT_TABS.map((tab) => (
                <button
                  ...
                  style={
                    activeTab === tab
                      ? { backgroundColor: "rgba(255,255,255,0.15)", borderColor: "rgba(255,255,255,0.4)" }
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              ...
              <button
                type="submit"
                className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl text-white font-heading font-bold text-sm transition-all duration-200 mt-1 mb-8 bg-accent hover:bg-[#163cb1] shadow-lg shadow-accent/20 border-none cursor-pointer"
              >
      ...
      <section className="w-full py-16 px-5 sm:px-8 bg-white">
        <div className="max-w-6xl mx-auto">
          <p className="text-xs font-heading font-bold tracking-[2px] uppercase mb-2" style={{ color: "#0D1F3C" }}>
            More ways to reach us
          </p>
          <h2 className="text-2xl sm:text-3xl font-heading font-extrabold mb-10" style={{ color: "#0D1F3C" }}>
            Other Ways to Connect
          </h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {connectCards.map(({ icon, title, desc, btnLabel, href, bgStyle }) => {
              ...
              return (
                <div
                  key={title}
                  className="flex flex-col items-center text-center rounded-2xl px-5 py-7 gap-4 hover:-translate-y-1 transition-all duration-300 h-full border border-white/5 shadow-lg shadow-black/5"
                  style={{ backgroundColor: "#0D1F3C" }}
                >
                  <div
                  ...
                  <a
                    ...
                    className={`inline-flex items-center justify-center gap-1.5 w-full px-4 py-2.5 rounded-lg text-white text-xs font-heading font-bold transition-all duration-200 mt-1 hover:-translate-y-0.5 ${isWhatsApp
                        ...
                        : "bg-accent hover:bg-[#163cb1] shadow-lg shadow-accent/20"
                      }`}
                  >
      ...
      <section className="w-full py-6 px-5 sm:px-8 bg-white">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-5">
          ...
          {/* Social card */}
          <div className="flex-1 rounded-2xl px-8 py-9 border border-white/5" style={{ backgroundColor: "#0D1F3C" }}>
            <h3 className="text-white text-lg font-heading font-extrabold mb-1.5">
              Connect on social media
            </h3>
            <div className="w-12 h-0.5 rounded-full mb-8" style={{ backgroundColor: "rgba(255,255,255,0.3)" }} />
          ...
          {/* Quote card */}
          <div
            className="lg:w-80 flex-shrink-0 rounded-2xl px-8 py-9 flex flex-col relative"
            style={{ backgroundColor: "#0D1F3C" }}
          >
            <span className="text-5xl font-serif leading-none text-white/40 mb-2">❝</span>
            ...
            <p className="text-sm font-bold text-white/70 mt-5">— Pavan Bhattad</p>
            <span className="absolute bottom-6 right-7 text-4xl font-serif text-white/40">❞</span>
          </div>
        </div>
      </section>
      ...
      <section className="w-full py-6 px-5 sm:px-8 pb-16 bg-white">
        <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-5">
          ...
          {/* CTA */}
          <div
            className="flex-1 rounded-2xl px-10 py-12 flex flex-col gap-6 border border-white/5"
            style={{
              background: "linear-gradient(135deg, #0D1F3C 0%, rgba(13,31,60,0.9) 100%)",
            }}
          >
            <h3 className="text-3xl sm:text-4xl font-heading font-extrabold text-white leading-snug">
              Let's learn, grow<br />
              and{" "}
              <em className="not-italic text-white/80">transform</em>{" "}
              together
            </h3>
          ...
          {/* Programs */}
          <div
            className="flex-1 rounded-2xl px-10 py-12 bg-white transition-all duration-300 hover:shadow-xl hover:shadow-black/5"
            ...
            <div className="flex flex-col gap-6">
              {programs.map(({ icon, title, desc, bgStyle }) => (
                <div key={title} className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-heading font-bold text-black mb-0.5">{title}</p>
